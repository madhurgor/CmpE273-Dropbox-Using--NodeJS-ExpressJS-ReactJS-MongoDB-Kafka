...
function handle_request(msg, callback){
  ...
  msg.files.forEach((file) => {
     ...
     while(true)
     {
       if(!fs.existsSync(path.join(__dirname,'../../') + `/${msg.username}/normal/` + file.originalname))
       {
         fs.rename(path.join(__dirname,'../../') + '/uploads/' + file.filename, path.join(__dirname,'../../') + `/${msg.username}/normal/` + file.originalname, function(err) {
